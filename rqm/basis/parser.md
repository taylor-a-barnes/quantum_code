...
- `parse_basis(path: &Path) -> Result<BasisSet, ParseError>`
...
- `load_basis(element: &str, basis_name: &str) -> Result<BasisSet, LoadError>`
...
- `BasisSet` — the parsed representation of a single-element basis set:
...
- `ElectronShell` — a single contracted Gaussian shell:
...
- `ParseError` — error type returned by `parse_basis`:
